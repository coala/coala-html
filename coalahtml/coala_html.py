...
from coalahtml.helper import (
    ...
    get_file, create_dir, copy_files, get_args, build_file_tree)
...
def main():
    ...
    if args.noupdate is False:
        ...
        file_tree_data = get_file(CONFIGS['files'], dir_path)

        with open(file_tree_data, 'w') as fp:
            file_tree = build_file_tree()
            json.dump(file_tree, fp,
                      cls=JSONEncoder,
                      sort_keys=True,
                      indent=2,
                      separators=(',', ': '))
    ...
    if args.nolaunch is False:
        ...
        if not os.path.exists('bower_components'):
            res = call(['bower','install']) # returns 0 on successul call
            if res:
