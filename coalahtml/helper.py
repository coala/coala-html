...
def build_file_tree(current_dir: str = os.curdir):
    """
    List files recursively using breadth first search
    param current_dir:
        The current directory
    :return:
        A result after tree traversal

    """
    result = {}
    for root, dirs, files in list(os.walk(current_dir)):
        root = os.path.relpath(root, current_dir)
        if root.startswith('.') and not(root is current_dir):
            # Exclude the hidden base
            continue

        dirs = [os.path.relpath(i, current_dir) for i in dirs]
        files = [os.path.relpath(i, current_dir) for i in files]

        dir_file = dirs + files
        # Exclude the hidden files
        if root is current_dir:
            result[''] = list(set(dir_file) - set(
                            [fl for fl in dir_file if fl.startswith('.')]))
        else:
            result[root] = list(set(dir_file) - set(
                                [fl for fl in dir_file if fl.startswith('.')]))
    return result
